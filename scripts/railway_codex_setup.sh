...
# Railway CLI versions may print either shell-friendly KEY=VALUE pairs or a table.
# Prefer KEY=VALUE output when supported; fail fast if tabular output is returned.
if railway variables --help 2>/dev/null | rg -- '--kv' >/dev/null 2>&1; then
  railway variables --environment "$ENVIRONMENT_ID" --kv > "$OUT_FILE"
else
  railway variables --environment "$ENVIRONMENT_ID" > "$OUT_FILE"
fi

if rg -n '^[[:space:]]*[╔║╚]' "$OUT_FILE" >/dev/null 2>&1; then
  echo "[ERROR] Output variabili non in formato KEY=VALUE (tabella rilevata)." >&2
  echo "        Esegui manualmente: railway variables --environment \"$ENVIRONMENT_ID\" --kv > \"$OUT_FILE\"" >&2
  exit 1
fi
